// autoconf-casefile-rs: Receipt schema for autoconf-rs forensic parity courts.
//
...
//
// Clean-room design: this crate defines only data schemas. It contains no
// implementation code derived from GNU Autoconf.

use serde::{Deserialize, Serialize};
use std::collections::HashMap;

/// Top-level receipt schema version marker.
pub const RECEIPT_SCHEMA: &str = "autoconf-rs-receipt-v1";
...
pub struct Receipt {
    ...
    /// Court identifier, e.g. "AC.CLI.AUTOCONF.1", "AC.M4.M4SUGAR.1"
    pub court: String,

    /// Verdict: "admitted_match", "admitted_divergence", "known_failure", etc.
    pub verdict: String,
...
pub struct ComparisonResult {
    /// "byte_exact", "class_location_match", "known_divergence", "intentional_divergence", "not_applicable"
    pub stdout: String,
    pub stderr: String,
...
pub struct Claim {
    pub court: String,
    pub surface: String,
    pub status: String, // "sealed", "partial", "unclaimed", "known_failure", "monitored", "permanent_nonclaim"
...
pub struct ClaimLadder {
    ...
    pub permanent_nonclaim_count: usize,
}
...
impl Receipt {
    ...
    pub fn verify(&self) -> Result<(), Vec<String>> {
        ...
        if self.verdict == "admitted_match" && self.comparison.stdout.is_empty() {
            errors.push("admitted_match verdict but no stdout comparison".into());
        }
...
impl ClaimLadder {
    ...
    pub fn recount(&mut self) {
        ...
        self.unclaimed_count = self
            ...
            .count();
        self.known_failure_count = self
            .claims
            .iter()
            .filter(|c| c.status == "known_failure")
            .count();
        self.monitored_count = self
            .claims
            .iter()
            .filter(|c| c.status == "monitored")
            .count();
        self.permanent_nonclaim_count = self
            .claims
            .iter()
            .filter(|c| c.status == "permanent_nonclaim")
            .count();
    }
}
...
mod tests {
    ...
    fn test_receipt_new_and_verify() {
        let mut r = Receipt::new("AC.CLI.AUTOCONF.1", "autoconf-rs matches oracle byte-exact");
    ...
    fn test_receipt_render() {
        let r = Receipt::new("AC.CLI.AUTOCONF.1", "byte-identical copy-through");
        let md = r.render();
        assert!(md.contains("AC.CLI.AUTOCONF.1"));
        assert!(md.contains("byte-identical copy-through"));
    }

    #[test]
    fn test_claim_ladder_recount() {
        let mut ladder = ClaimLadder::new();
        ladder.claims.push(Claim {
            court: "AC.ORACLE.1".into(),
            surface: "Oracle admission".into(),
            status: "sealed".into(),
            receipts: vec![],
            description: "Oracle admitted".into(),
            since_version: "0.1.0".into(),
        });
        ladder.claims.push(Claim {
            court: "AC.CLI.1".into(),
            surface: "CLI harness".into(),
            status: "unclaimed".into(),
            receipts: vec![],
            description: "Not yet implemented".into(),
            since_version: "0.1.0".into(),
        });
        ladder.recount();
        assert_eq!(ladder.sealed_count, 1);
        assert_eq!(ladder.unclaimed_count, 1);
    }
}
