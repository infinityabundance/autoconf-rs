...
pub fn generate_configure_body(state: &AutoconfState) -> Vec<u8> {
    ...
    b.extend_from_slice(b"cat >config.status <<_ACEOF\n");
    b.extend_from_slice(b"#! /bin/sh\n");
    b.extend_from_slice(b"# config.status\n");
    b.extend_from_slice(b"CONFIG_STATUS=./config.status\n");
    b.extend_from_slice(format!("PACKAGE_NAME='{}'\n", name).as_bytes());
    b.extend_from_slice(
        format!("# Generated by autoconf-rs for {} {}\n", name, version).as_bytes(),
    );
    b.extend_from_slice(b"_ACEOF\n\n");

    b.extend_from_slice(b"chmod +x config.status\n");
