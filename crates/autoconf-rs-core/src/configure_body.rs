...
pub fn generate_feature_test_body(state: &AutoconfState) -> Vec<u8> {
    ...
    // === confdefs.h === MUST be created before any probe: each successful AC_CHECK_* appends a
    // `#define HAVE_X 1` here, and config.status later bakes confdefs.h into config.h. (Previously
    // confdefs.h was created at the END of this function, after the probes -> `cat confdefs.h`
    // failed and config.h was built from an empty confdefs.h, so no HAVE_* were ever defined.)
    // NON-truncating: confdefs.h is already created in the prologue (so configure.ac-body probes that
    // run before this point can `cat confdefs.h`), and AC_DEFINEs append to it; truncating here would
    // discard those. Only create it if somehow still absent.
    b.extend_from_slice(b"test -f confdefs.h || printf '%s\\n' '/* confdefs.h */' > confdefs.h\n");
    ...
    b.extend_from_slice(b"test -f conf_subst.sed || : > conf_subst.sed\n");
    ...
    // command-not-found (exit 127) -> every probe took the "no" branch -> no HAVE_* ever defined.
    b.extend_from_slice(
        b"ac_ext=c\nac_objext=o\nac_exeext=\n\
...
ac_fn_c_try_compile () {\n\
  ...
  if eval \"$ac_compile\" >/dev/null 2>&1 && test -s conftest.$ac_objext; then ac_retval=0; else ac_retval=1; fi\n\
  rm -f conftest.err conftest.$ac_objext\n\
  return $ac_retval\n\
}\n\
ac_fn_c_try_link () {\n\
  rm -f conftest.$ac_objext conftest$ac_exeext\n\
  if eval \"$ac_link\" >/dev/null 2>&1 && test -s conftest$ac_exeext; then ac_retval=0; else ac_retval=1; fi\n\
  rm -f conftest.err conftest.$ac_objext conftest$ac_exeext\n\
  return $ac_retval\n\
}\n\n",
    );
    ...
    if state.has_ifelse_checks {
        ...
        b.extend_from_slice(b"cat confdefs.h 2>/dev/null - <<_ACEOF >conftest.$ac_ext\n");
    ...
    for hdr in &state.checked_headers {
        ...
        b.extend_from_slice(b"  printf '%s\\n' \"yes\"\n");
        b.extend_from_slice(
            format!("  printf '%s\\n' \"#define {} 1\" >>confdefs.h\n", have_macro(hdr)).as_bytes(),
        );
        b.extend_from_slice(b"else\n  printf '%s\\n' \"no\"\nfi\n\n");
    }

    // Library checks. AC_CHECK_LIB(lib, func): try linking func with -llib; on success define
    // HAVE_LIBlib and KEEP -llib in LIBS, on failure RESTORE LIBS (a failed probe lib must not
    // pollute the link line of every later target).
    for (lib, func) in &state.checked_libs {
        let have_lib = format!("HAVE_LIB{}", lib.to_ascii_uppercase());
        b.extend_from_slice(b"printf %s \"checking for ");
        b.extend_from_slice(func.as_bytes());
        b.extend_from_slice(b" in -l");
        b.extend_from_slice(lib.as_bytes());
        b.extend_from_slice(b"... \"\n");
        b.extend_from_slice(b"ac_check_lib_save_LIBS=$LIBS\n");
...
/// The C preprocessor macro a successful AC_CHECK_HEADER/FUNC defines: `sys/time.h` -> `HAVE_SYS_TIME_H`,
/// `strdup` -> `HAVE_STRDUP`. Non-alphanumeric chars become `_`; everything upper-cased.
pub fn have_macro(name: &str) -> String {
    let up: String = name
        .trim()
        .chars()
...
pub fn generate_configure_body(state: &AutoconfState) -> Vec<u8> {
    ...
    for h in &state.config_headers {
        // Build a sed script from confdefs.h (the RUNTIME probe results: every `#define HAVE_X 1`
        // an AC_CHECK_* appended). Each becomes `s|#undef X|#define X V|`, applied to the header
        // template so detected features actually land in config.h. This is how the HAVE_* defines
        // reach the header; the static `-e` seds below cover compile-time AC_DEFINE/PACKAGE values.
        b.extend_from_slice(
