...
-DPACKAGE_BUGREPORT=\\\\\\\"$PACKAGE_BUGREPORT\\\\\\\"\"\n\n",
    ...
    // ac_try MUST be double-quoted so it holds $ac_link's VALUE (`$CC -o …`); with single quotes it
    // stayed the literal `$ac_link` and `eval \"$ac_try\"` expanded only one level -> `$CC` ran as a literal
    // command (`$CC: command not found`, postgres's AC_RUN_IFELSE \"checking test program\" probe).
